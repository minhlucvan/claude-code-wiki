...
When project branding is active (loaded during Pre-check), append to every prompt:

```
BRANDING:
- PALETTE: [colors from branding guide, e.g., Primary #1A1A1A, Accent #A8D4F0, ...]
- ELEMENTS: [visual element rules, e.g., single-weight hand-drawn lines, basic geometric shapes]
- CONSTRAINTS: [do's and don'ts from branding guide]
```

Branding colors override the `COLORS` line in type templates. Branding style rules take precedence over style spec where they conflict.
